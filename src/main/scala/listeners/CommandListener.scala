...
class CommandListener(requests: Requests) extends CommandController(requests) {
  ...
  private val getCharacter: MyCommand = new GetCharacter
  // host commands
  ...
  private def getEnabledNamedCommands: List[NamedCommand[_]] = {
    ...
    val storymapleEnabledCommands = List(rank, rankNumber, guild, whodrops, whatdropsfrom, bossroster, rissadk, getCharacter)
